...
export function projectMonthEnd(state, referenceDate = new Date()) {
  ...
  return {
    ...
    confidence,
    warnings: [
      confirmedIncome <= 0 ? 'Renda em dinheiro não cadastrada para o período.' : null,
      day < 10 ? 'Projeção ainda no início do mês.' : null,
    ].filter(Boolean),
  }
}
...
export function calculateSafeInvestmentCapacity(state, referenceDate = new Date()) {
  ...
  return {
    ...
    reasons: [
      ...
      goalCommitment > 0 ? `Metas do mês preservadas: R$ ${round(goalCommitment).toFixed(2)}.` : null,
    ].filter(Boolean),
  }
}
...
export function buildPredictiveSnapshot(state, referenceDate = new Date()) {
  ...
  const mainRisk = overrunRisks[0]
    ...
    : projection.projectedBalance < 0
      ? 'Fechamento do mês pode ficar negativo.'
...
export function buildCutRecommendations(snapshot) {
  ...
  return risks.slice(0, 3).map((risk) => {
    ...
    return {
      ...
      reason: `${risk.category} está projetada acima da referência determinística.`,
    }
  })
}
...
export function validateAdvisorFeedback(input = {}) {
  ...
  if (!recommendationId) throw new Error('recommendation_id obrigatório')
  if (!ADVISOR_FEEDBACK_VALUES.includes(value)) throw new Error('feedback inválido')
  return {
