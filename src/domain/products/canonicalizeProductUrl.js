...
export class ProductIdentityMismatchError extends Error {
  constructor(message = 'Não conseguimos confirmar que o anúncio encontrado é o mesmo produto do link informado.') {
    super(message)
    this.name = 'ProductIdentityMismatchError'
    this.code = 'PRODUCT_IDENTITY_MISMATCH_BLOCKED'
    this.status = 409
  }
}
...
function compactUrlCandidate(value) {
  return String(value || '')
    .trim()
    .replace(/[)\].,;]+$/g, '')
}

export function extractProductUrls(input) {
  const text = String(input || '')
  const urls = []
  const starts = [...text.matchAll(/https?:\/\//gi)].map((match) => match.index).filter((index) => index != null)
...
function removeParams(url, blockedParams) {
  const removedParams = []
...
function marketplaceLabelForHost(host) {
...
function isTechnicalEndpoint(url) {
...
export function extractMercadoLivreProductId(input) {
  const raw = safeDecode(String(input || ''))
...
function baseIdentity({ source, canonicalUrl, sourceProductId, idType, rawUrl, removedParams, parsed }) {
  return {
    source,
    ...
    id_type: idType,
    idType,
    marketplace: MARKETPLACE_LABELS[source] || source,
...
export function canonicalizeProductUrl(input) {
  ...
  if (isTechnicalEndpoint(parsed)) {
    return {
      ...
      message: 'Cole o link da página do produto, não um endpoint técnico do sistema.',
      raw_url: rawUrl,
    }
  }
  ...
  if (host.includes('amazon.com.br') || host.includes('amazon.com')) {
    ...
    if (!asin) {
      return {
        ...
        message: 'Não foi possível identificar o ASIN do produto Amazon.',
        raw_url: rawUrl,
      }
    }
  ...
  if (host.includes('mercadolivre.com.br') || host.includes('mercadolibre.com')) {
    ...
    if (!identity?.source_product_id) {
      return {
        ...
        message: 'Não foi possível identificar o produto do Mercado Livre.',
        raw_url: rawUrl,
      }
    }
...
export function productCacheKey(identity) {
...
function titleLooksCompatible(expectedTitle, candidateTitle) {
...
export function assertProductIdentityMatch(inputIdentity, extractedProduct) {
  ...
  const expectedTitle = inputIdentity?.title || inputIdentity?.name || ''
  const candidateTitle = extractedProduct?.title || extractedProduct?.name || ''
...
export function classifyProductCandidateForIdentity(identity, candidate) {
  try {
