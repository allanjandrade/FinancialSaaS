...
if (!baseUrl || /^https?:\/\/(localhost|127\.0\.0\.1)(:|\/|$)/i.test(baseUrl)) {
  throw new Error('Defina RELEASE8_REMOTE_APP_URL ou CYPRESS_BASE_URL com a URL publicada para rodar o smoke remoto da Release 8.3.')
}
...
child.on('exit', (code) => {
  if (code === 0) console.log('Release 8.3 smoke remoto: PASS, navegação profissional validada')
  process.exit(code ?? 1)
})
