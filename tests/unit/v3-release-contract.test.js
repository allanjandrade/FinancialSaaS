...
describe('Version 3.4.0 release contract', () => {
  it('declares version 3.4.0 in package metadata and runtime app config', () => {
    const pkg = JSON.parse(read('package.json'))
    const lock = JSON.parse(read('package-lock.json'))

    expect(pkg.version).toBe('3.4.0')
    expect(lock.version).toBe('3.4.0')
    expect(lock.packages[''].version).toBe('3.4.0')

    expect(exists('src/config/app-version.js')).toBe(true)
    const appVersion = read('src/config/app-version.js')
    expect(appVersion).toContain("APP_VERSION = '3.4.0'")
  ...
  it('adds a dedicated v3 release validator and documentation trail', () => {
    ...
    expect(read('scripts/validate-v3-release.js')).toContain("const currentVersion = '3.4.0'")
